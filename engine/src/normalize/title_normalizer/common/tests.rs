...
mod integration_tests {
    use super::*;
    use crate::normalize::title_normalizer::common::*;
    use regex::Regex;
    ...
    fn test_complete_common_module_integration() {
        ...
        // Test comparators
        assert_eq!(
            case_insensitive_cmp("apple", "Banana"),
            std::cmp::Ordering::Less
        );
        assert_eq!(
            natural_cmp("file1.txt", "file10.txt"),
            std::cmp::Ordering::Less
        );
    ...
    fn test_quantity_comprehensive() {
        ...
        assert!(matches!(
            parse_quantity("30 fps"),
            Some(Quantity::FrameRate(_))
        ));
        assert!(matches!(
            parse_quantity("320kbps"),
            Some(Quantity::BitRate(_))
        ));
        assert!(matches!(
            parse_quantity("5 min"),
            Some(Quantity::Duration(_))
        ));
    }
    ...
    fn test_comparators_comprehensive() {
        ...
        assert_eq!(length_cmp("short", "longer"), std::cmp::Ordering::Less);
        assert_eq!(
            length_reverse_cmp("short", "longer"),
            std::cmp::Ordering::Greater
        );
        assert_eq!(
            numeric_or_lexical_cmp("10", "2"),
            std::cmp::Ordering::Greater
        );
        assert_eq!(
            word_count_cmp("one two", "one"),
            std::cmp::Ordering::Greater
        );
        ...
        assert_eq!(
            natural_cmp("version1.2", "version1.10"),
            std::cmp::Ordering::Less
        );
        ...
        assert_eq!(
            list_position_cmp(&list, &"apple", &"cherry"),
            std::cmp::Ordering::Less
        );

        // Test similarity comparison
        assert_eq!(
            similarity_cmp("hello", "hello", "world"),
            std::cmp::Ordering::Less
        );
        ...
        assert_eq!(
            pattern_match_cmp(&pattern, "123", "abc"),
            std::cmp::Ordering::Less
        );
        ...
        assert_eq!(
            priority_cmp(&priority, "high", "low"),
            std::cmp::Ordering::Less
        );
    }
